...
export class CatalogueService {
  ...
  async getTrendingProducts(): Promise<any> {
    ...
    if (this.activeFetches.has(cacheKey)) {
      console.log(
        '🔒 [MUTEX HIT] Coalescing request. Waiting for the active database fetch to finish...',
      );
    ...
    console.log(
      '⚠️ [CACHE MISS] No active fetch found. Initiating database query...',
    );

    const fetchPromise = this.simulateHeavyDatabaseQuery()
      .then(async (dbData) => {
